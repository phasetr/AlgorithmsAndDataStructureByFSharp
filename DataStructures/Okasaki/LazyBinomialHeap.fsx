...
let rec insertTree: BinomialTree<'a> -> BinomialTree<'a> list -> BinomialTree<'a> list when 'a: comparison =
  fun t -> function
    | [] -> [t]
...
let singleton: 'a -> Lazy<BinomialTree<'a> list> = fun x -> lazy [singletonTree x]
...
let rec mrg: BinomialTree<'a> list -> BinomialTree<'a> list -> BinomialTree<'a> list when 'a: comparison = fun h1 h2 ->
  match h1, h2 with
  | [], x | x, [] -> x
  | hd1::tl1, hd2::tl2 ->
...
let merge: t<'a> -> t<'a> -> Lazy<BinomialTree<'a> list> when 'a: comparison = fun h1 h2 ->
...
let findMin: BinomialTree<'a> list -> 'a when 'a: comparison = fun h ->
