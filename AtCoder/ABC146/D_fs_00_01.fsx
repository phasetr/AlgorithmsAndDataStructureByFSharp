...
let solve N Ia =
  ...
  let rec dfs pi ci color Xd =
    let Cq = Ga.[ci] |> List.filter (snd >> (<>) pi) |> Seq.zip (Seq.initInfinite ((+) 1) |> Seq.filter ((<>) color))
    (Cq, Xd) ||> Seq.foldBack (fun (color,(j,gci)) Xd -> dfs ci gci color (Xd |> Map.add j color))
  Map.empty |> dfs 0 0 0
  // |> fun Xd -> let vq = Xd.Values in Seq.append (seq { Seq.max vq }) vq // Map.ValuesはF#6から
  |> fun Xd -> let vq = Xd |> Map.toSeq |> Seq.map snd in Seq.append (seq { Seq.max vq }) vq
