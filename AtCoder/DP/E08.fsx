...
let N,W,wva = 1,1000000000L,[|(1000000000L,10)|]
// From E06.hs
let solve N W wva =
    let inf = 1_000_000_001
    let f (v0:int64[]) (wi,vi) =
        let m j = if j < vi then min v0.[j] wi else min v0.[j] (wi+v0.[j-vi])
        Array.map m [|0..100000|]
    Array.fold f (Array.replicate 100001 inf) wva
    |> Array.takeWhile (fun wi -> wi<=W) |> Array.length
