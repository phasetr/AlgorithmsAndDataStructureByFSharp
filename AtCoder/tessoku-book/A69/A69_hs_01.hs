-- https://atcoder.jp/contests/tessoku-book/submissions/35838714
...
main = do
  ...
  let abc = ps ++ abc' where
        ij = [(i,j) |i<-[1..n'], j<-[n'+1..n'+n']]
        ss' = map (\c -> if c == '#' then 1 else 0) ss
        abc' = zipWith (\(a,b) c -> (a,b,c)) ij ss'
        ps = concat [[(0,i,1), (n'+i,n-1,1)] | i <- [1..n']]
  ...
  let path res = foldl func (10000, []) <$> back t where
        back k | k < 0 = Nothing
               | k == 0 = Just []
