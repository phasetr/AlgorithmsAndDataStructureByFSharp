// https://atcoder.jp/contests/tessoku-book/submissions/37675790
using System;
...
class Program
{
    ...
    static void Solve()
    {
        ...
        var len = Enumerable.Repeat(long.MaxValue, n).ToArray();
        len[0] = 0;
        ...
        {
            var cur = que.Dequeue();
            foreach (var prev in tree[cur])
            {
                if (visits[prev.to]) continue;
                if (len[prev.to] + prev.len == len[cur])
                {
                    visits[prev.to] = true;
                    que.Enqueue(prev.to);
                }
