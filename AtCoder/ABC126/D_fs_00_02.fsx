...
let solve N Ia =
  let Aa =
    (Array.init N (fun _ -> ResizeArray<int*int>()),Ia)
    ||> Array.fold (fun Aa (u,v,w) ->
      Aa.[u-1].Add((v-1,w&&&1)); Aa.[v-1].Add((u-1,w&&&1)); Aa)
    |> Array.map (fun v -> v.ToArray())

  let rec dfs pi ci v Xa =
    Array.set Xa ci (v^^^1)
    Array.get Aa ci
    |> Array.filter (fun (i,_) -> i <> pi)
    |> Array.fold (fun acc (gci,w) -> dfs ci gci (v^^^w) acc) Xa
